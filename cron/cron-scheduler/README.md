...
- After installing, please run `npm run start:dev:workers`. It will start the workers in watch mode.

## How to run application in production mode
- First copy .env.sample file to .env and fill the all envs
- Then run `npm i` to install all the dependencies
- Run `make build` to build the project
- After installing, please run `npm run start` to start the server
- After installing, please run `npm run start:workers`. It will start the workers.
