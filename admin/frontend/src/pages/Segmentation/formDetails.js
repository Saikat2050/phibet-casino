...
const fieldValidationSchema = Yup.object({
	...
	value1: Yup.mixed()
		...
		.test('value1-valid', 'Invalid date or number', (value) => {
	...
	value2: Yup.mixed()
		.nullable()
		.test('value2-required', 'Value2 is required', (value) => {
			const { operator } = this.parent;
			return isValueRequired(operator, FIELD_TYPES.DOUBLE) ? !!value : true;
		})
		.test('value2-greater', 'Value2 must be greater than Value1', (value) => {
...
const validationSchema = Yup.object({
	...
	groups: Yup.array()
		...
		.test(
			...
			(groups) => validateUniqueCombinations(groups, this)
		)
...
export { validationSchema, filterValues, filterValidationSchema };
