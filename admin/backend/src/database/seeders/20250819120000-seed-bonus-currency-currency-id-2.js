...
async function up (queryInterface, Sequelize) {
  console.log('Starting bonus currency seeder for currency ID 2...')
  ...
  // Step 2: Check if currency with ID 2 exists
  try {
    const currency = await queryInterface.sequelize.query(
      'SELECT id FROM currencies WHERE id = 2',
      {
        type: Sequelize.QueryTypes.SELECT,
        transaction
      }
    ...
    if (currency.length === 0) {
      console.log('❌ Currency with ID 2 does not exist, rolling back to prevent migration entry')
      await transaction.rollback()
      throw new Error('Currency with ID 2 does not exist')
    }
    
    console.log('✅ Currency with ID 2 exists')
  } catch (error) {
    if (!transaction.finished) {
      await transaction.rollback()
    }
  ...
  // Step 4: Get existing bonus currency records for currency ID 2
  let existingBonusIds = []
  try {
    const existingBonusCurrencies = await queryInterface.sequelize.query(
      'SELECT bonus_id FROM bonus_currencies WHERE currency_id = 2',
      {
        type: Sequelize.QueryTypes.SELECT,
        transaction
      }
    ...
    console.log(`✅ Found ${existingBonusIds.length} existing bonus currency records for currency ID 2`)
  } catch (error) {
    await transaction.rollback()
  ...
  // Step 5: Filter bonuses that don't have currency ID 2 records
  const bonusesToAdd = bonuses.filter(bonus => !existingBonusIds.includes(bonus.id))

  if (bonusesToAdd.length === 0) {
    console.log('✅ All bonuses already have currency ID 2 records, nothing to add')
  ...
  // Step 6: Create bonus currency records for currency ID 2
  try {
    const now = new Date()
    ...
    console.log(`✅ Successfully added ${bonusCurrencyData.length} bonus currency records for currency ID 2`)
  } catch (error) {
    await transaction.rollback()
...
async function down (queryInterface) {
  console.log('Starting rollback of bonus currency seeder for currency ID 2...')
  ...
  try {
    // Remove all bonus currency records for currency ID 2
    ...
    await transaction.commit()
    console.log(`✅ Successfully removed bonus currency records for currency ID 2 (${result} records affected)`)
  } catch (error) {
    await transaction.rollback()
