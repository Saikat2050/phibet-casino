...
const constraints = ajv.compile({
  ...
  properties: {
    userId: { type: 'string' },
    depositLimit: { type: ['string', 'number'] },
...
/**
 * Service to update a user's deposit limit (daily, weekly, or monthly).
...
export class UpdateDepositLimitService extends ServiceBase {
  ...
  /**
   * Updates the deposit limit for a user for the specified time period.
   * @returns {Promise<{ userLimit: object }>} The updated userLimit row.
   */
  async run () {
    const { userId, depositLimit, timePeriod, reset, adminUserId } = this.args
    ...
    try {
      ...
      const limitData = reset ? { value: '' } : { value: depositLimit }
      ...
      logAdminActivity({
        ...
        previousData: { depositLimits: previousData },
        modifiedData: { depositLimits: modifiedData },
        service: 'updateDepositLimit',
