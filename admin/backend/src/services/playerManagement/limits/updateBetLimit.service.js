...
const constraints = ajv.compile({
  ...
  properties: {
    userId: { type: 'string' },
    betLimit: { type: ['string', 'number'] },
    timePeriod: { type: 'string', enum: ['daily', 'weekly', 'monthly'] },
    reset: { type: 'boolean' },
    type: { type: 'string', enum: ['bet'] },
    adminUserId: { type: ['string', 'number'], nullable: true }
  },
  required: ['userId', 'betLimit', 'timePeriod']
})

/**
 * Service to update a user's bet limit (daily, weekly, or monthly).
 * Updates the user_limits table for the specified user and time period.
 */
export class UpdateBetLimitService extends ServiceBase {
  /**
  ...
  /**
   * Updates the bet limit for a user for the specified time period.
   * @returns {Promise<{ userLimit: object }>} The updated userLimit row.
   */
  async run () {
    const { userId, betLimit, timePeriod, reset, adminUserId } = this.args
    ...
    switch (timePeriod) {
      case 'daily':
        key = USER_RESPONSIBLE_GAMBLING_LIMIT_TYPES.DAILY_BET_LIMIT
        break
      case 'weekly':
        key = USER_RESPONSIBLE_GAMBLING_LIMIT_TYPES.WEEKLY_BET_LIMIT
        break
      case 'monthly':
        key = USER_RESPONSIBLE_GAMBLING_LIMIT_TYPES.MONTHLY_BET_LIMIT
        break
      default:
    ...
    try {
      ...
      const limitData = reset ? { value: '' } : { value: betLimit }
      ...
      logAdminActivity({
        ...
        previousData: { betLimits: previousData },
        modifiedData: { betLimits: modifiedData },
        service: 'updateBetLimit',
