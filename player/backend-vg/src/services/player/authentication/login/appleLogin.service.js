...
export class AppleLoginService extends ServiceBase {
  async run () {
    const {
      dbModels: {
        User: UserModel,
    ...
    try {
      ...
      if (isSignup) {
        ...
        const newUser = {
          email,
          currencyCode: 'USD',
          signInMethod: SIGN_IN_METHOD.APPLE,
          isEmailVerified: true,
          kycStatus: KYC_STATUS.ACCOUNT_EMAIL_VERIFIED,
          isTermsAccepted: isTermsAccepted,
          signInIp: ipAddress,
          moreDetails: {
            loginMethod: SIGN_UP_METHOD.APPLE,
            browser,
            platform,
            isSubscribed: true,
        ...
        console.log('>>>>>>>>>>>>>>>>>>>>>>>sokul for apple signup', sokulData)
      ...
      if (user?.signInMethod !== SIGN_IN_METHOD.APPLE) return this.addError('UseAppropriateLoginMethodErrorType')
      ...
      console.log('Error while logging from Apple', error)
