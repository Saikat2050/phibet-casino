...
export default class ServiceBase {
  ...
  addError (attribute, errorMessage) {
    ...
    // Do not prefix for error types (ending with ErrorType)
    const isErrorType = attribute.endsWith('ErrorType')
    const message = isErrorType ? errorMessage : `${_.startCase(attribute)} ${errorMessage}`
    if (!errors[attribute]) {
      _.extend(errors, { [attribute]: message })
    } else {
