  ...
  if (success) {
    ...
    if (!_.isEmpty(errors)) {
      ...
      if (_.has(errorValues[0], 'validationErrors')) {
        ...
        const serviceName = Object.keys(errors)[0]
        const serviceErrors = errors[serviceName]
        const errorList = Object.entries(serviceErrors).map(([errorType, errorMsg]) => {
          const errorConstant = errorTypes[errorType] || {}
          return {
            name: errorType,
            errorCode: errorConstant.errorCode,
            description: errorMsg || errorConstant.description,
            statusCode: errorConstant.statusCode || 400,
            isOperational: errorConstant.isOperational !== undefined ? errorConstant.isOperational : true
          }
        })
        next(errorList)
      }
