...
export class ResendMobileOtpService extends ServiceBase {
  get constraints () {
    return constraints
  }
  ...
  async run () {
    ...
    try {
      const userDetails = await userModel.findOne({ where: { id: userId }, attributes: ['id', 'phoneVerified', 'phone', 'newOtpRequested', 'phoneOtp', 'phoneCode'], transaction })
      ...
      const otpRequestedTime = new Date(userDetails.newOtpRequested).getTime()
      const currentTime = new Date().getTime()
      const diff = (currentTime - otpRequestedTime) / 1000 // converting into the seconds
      ...
      return { success: true, data: {}, message: SUCCESS_MSG.OTP_RESENT }
    } catch (error) {
