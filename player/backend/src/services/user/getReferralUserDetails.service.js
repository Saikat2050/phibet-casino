...
export class GetReferralUserDetailsService extends ServiceBase {
  get constraints () {
    return constraints
  }
  ...
  async run () {
    try {
      ...
      return { count: referredUser.count, referredUser: referredUser.rows, page, totalPages: Math.ceil(referredUser.count / perPage) }
    } catch (err) {
