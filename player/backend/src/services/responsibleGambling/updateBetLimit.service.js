...
/**
 * Service to update or reset bet limits (daily, weekly, monthly) for a user.
...
export class UpdateBetLimitService extends ServiceBase {
  ...
  /**
   * Updates or resets bet limits for a user.
  ...
  async run () {
    ...
    try {
      ...
      const keys = [USER_RESPONSIBLE_GAMBLING_LIMIT_TYPES.DAILY_BET_LIMIT, USER_RESPONSIBLE_GAMBLING_LIMIT_TYPES.WEEKLY_BET_LIMIT, USER_RESPONSIBLE_GAMBLING_LIMIT_TYPES.MONTHLY_BET_LIMIT]
      ...
      for (const limit of limits) {
        if (reset) {
          ...
          if (limit.key === USER_RESPONSIBLE_GAMBLING_LIMIT_TYPES.DAILY_BET_LIMIT) limit.value = daily
          if (limit.key === USER_RESPONSIBLE_GAMBLING_LIMIT_TYPES.WEEKLY_BET_LIMIT) limit.value = weekly
          if (limit.key === USER_RESPONSIBLE_GAMBLING_LIMIT_TYPES.MONTHLY_BET_LIMIT) limit.value = monthly
