...
/**
 * Service to update or reset deposit limits (daily, weekly, monthly) for a user.
 * Enforces password verification, limit relationship validation, and 24h cooldown on updates/resets.
 */
export class UpdateDepositLimitService extends ServiceBase {
  get constraints () {
    return constraints
  }

  /**
   * Updates or resets deposit limits for a user.
  ...
  async run () {
    ...
    try {
      ...
      const keys = [USER_RESPONSIBLE_GAMBLING_LIMIT_TYPES.DAILY_DEPOSIT_LIMIT, USER_RESPONSIBLE_GAMBLING_LIMIT_TYPES.WEEKLY_DEPOSIT_LIMIT, USER_RESPONSIBLE_GAMBLING_LIMIT_TYPES.MONTHLY_DEPOSIT_LIMIT]
      ...
      for (const limit of limits) {
        if (reset) {
          ...
          if (limit.key === USER_RESPONSIBLE_GAMBLING_LIMIT_TYPES.DAILY_DEPOSIT_LIMIT) limit.value = daily
          if (limit.key === USER_RESPONSIBLE_GAMBLING_LIMIT_TYPES.WEEKLY_DEPOSIT_LIMIT) limit.value = weekly
          if (limit.key === USER_RESPONSIBLE_GAMBLING_LIMIT_TYPES.MONTHLY_DEPOSIT_LIMIT) limit.value = monthly
